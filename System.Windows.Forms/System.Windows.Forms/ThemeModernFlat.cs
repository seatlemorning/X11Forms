...
namespace System.Windows.Forms
{
    internal class ThemeModernFlat : ThemeWin32Classic
    {
        ...
        // ===== LEFT ALIGNED StringFormat for RadioButton/CheckBox =====
        private static readonly StringFormat left_format = new StringFormat
        {
            Alignment = StringAlignment.Near,
            LineAlignment = StringAlignment.Center,
            HotkeyPrefix = System.Drawing.Text.HotkeyPrefix.Show,
            Trimming = StringTrimming.None, // No ellipsis - full text display
            FormatFlags = StringFormatFlags.NoWrap
        };
            ...
            Rectangle clipRectangle, bool isFlat)
        {
            ...
            using (var path = GetRoundedRectangle(drawRect, radius))
            {
                ...
                {
                    using (var brush =
                           new LinearGradientBrush(drawRect, backColor1, backColor2, LinearGradientMode.Vertical))
                    {
                ...
                if (!isFlat && !isDisabled && !isPressed)
                {
                               ...
                               LinearGradientMode.Vertical))
                    {
                        using (var highlightPath = GetRoundedRectangle(
                                   new Rectangle(drawRect.X + 1, drawRect.Y + 1, drawRect.Width - 2,
                                       drawRect.Height / 3),
                                   Math.Max(radius - 1, 1)))
                        {
            ...
            if (!string.IsNullOrEmpty(button.Text) && textBounds.Width > 0 && textBounds.Height > 0)
            {
                if (!isFlat && !isDisabled && (isDefault || isPressed))
                {
                    ...
                    {
                        var shadowBounds = new Rectangle(textBounds.X + 1, textBounds.Y + 1, textBounds.Width,
                            textBounds.Height);
...
// Рамка вокруг области содержимого
            ...
            if (!string.IsNullOrEmpty(cb.Text))
            {
                ...
                using (var brush = new SolidBrush(color))
                {
                    var textRect = new Rectangle(rect.Right + 8, textBounds.Y,
                        ...
                        textBounds.Height);
                    g.DrawString(cb.Text, cb.Font, brush, textRect, left_format);
                }
            }
        }
            ...
            if (!string.IsNullOrEmpty(rb.Text))
            {
                ...
                using (var brush = new SolidBrush(color))
                {
                    var textRect = new Rectangle(rect.Right + 8, textBounds.Y,
                        Math.Max(5, textBounds.Width - rect.Width - 8),
                        textBounds.Height);
                    g.DrawString(rb.Text, rb.Font, brush, textRect, left_format);
                }
            }
        }
        ...
        public override void DrawGroupBox(Graphics dc, Rectangle area, GroupBox box)
        {
            ...
            {
                // Measure text with proper sizing
                SizeF textSize = g.MeasureString(text, font, rect.Width - 20, format);
                int textWidth = (int)textSize.Width + 12;
                int textHeight = (int)textSize.Height;

                // Ensure minimum width for text
                textWidth = Math.Max(textWidth, 30);

                // If text is too wide for the groupbox, use the available space
                ...
                // Exclude text area from clipping to draw border behind text
                using (var textRegion = new Region(new Rectangle(rect.X + xPos - 4, 0, textWidth + 8, textHeight + 2)))
                {
                    g.Clip = textRegion;
                    g.Clip = prevClip; // Restore
                }

                // Draw border using the base method but with flat style
                using (var pen = new Pen(BorderColor, 1))
                {
                ...
                // Draw text with background to hide the line behind it
                if (!string.IsNullOrEmpty(text))
                {
                    var color = box.Enabled ? PrimaryColor : TextDisabled;

                    // Background for text (white rectangle to hide the line)
        ...
        public override void ComboBoxDrawBackground(ComboBox comboBox, Graphics g, Rectangle clippingArea,
            FlatStyle style)
        {
            base.ComboBoxDrawBackground(comboBox, g, clippingArea, style);
        }

        public override void ComboBoxDrawNormalDropDownButton(ComboBox comboBox, Graphics g, Rectangle clippingArea,
            Rectangle area, ButtonState state)
        {
